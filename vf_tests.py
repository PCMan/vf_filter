...
def create_csv_fields(classifier, select_feature_names, label_encoder, perform_rfe):
    csv_fields = ["iter"]
...
def main(args):
    ...
    csv_fields = create_csv_fields(classifier, selected_feature_names, label_encoder,
                                   perform_rfe=(args.perform_rfe or svm_rfe_classifier))
    with open(args.output, "w", newline="", buffering=1) as f:  # buffering=1 means line buffering
        ...
        for it in range(1, n_test_iters + 1):
            ...
            if classifier.perform_rfe or classifier.filter_fs_order:
